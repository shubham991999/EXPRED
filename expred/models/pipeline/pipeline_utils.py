...
def make_token_preds_batch(classifier: nn.Module,
    ...
    targets, queries, sentences = zip(*[(s.kls, s.query, s.sentence) for s in batch_elements])
    ids = [(s.ann_id, s.docid, s.index) for s in batch_elements]
    targets = PaddedSequence.autopad([torch.tensor(t, dtype=torch.long, device=device) for t in targets],
...
def score_token_rationales(truth: object,
                           documents: object,
                           input_data: object,
                           token_mapping: object,
                           hard_annotations: object,
                           soft_annotations: object
                           ) -> object:
    results = {}
    doc_soft_annotations = dict()  # (annid, docid) -> [token soft annotations]
    doc_hard_annotations = dict()
    for ann, soft_ann, hard_ann in zip(input_data, soft_annotations, hard_annotations):
        k = (ann[1].ann_id, ann[1].docid)
        overhead_len = len(
            ann[1].query) + 2  # remove overhead such as [cls], [sep] and the precussing query sub-tokens after [sep]
        doc_soft_annotations[k] = soft_ann[overhead_len:]
        doc_hard_annotations[k] = hard_ann[overhead_len:]

    def flatten_token_mapping(mapping):
        ret = []
        for token_mapping_per_sent in mapping:
            offset = 0 if len(ret) == 0 else ret[-1][1]
            for start, end in token_mapping_per_sent:
                ret.append((start + offset, end + offset))
        return ret

    flattened_token_mapping = {docid: flatten_token_mapping(mapping)
                               for docid, mapping in token_mapping.items()}

    def collapse_subtokens(sub_token_scores, token_mapping):
        ret = []
        for mapping in token_mapping:
            start_sub_token, end_sub_token = mapping
            ret.append(max(sub_token_scores[start_sub_token: end_sub_token]))
        return ret

    doc_soft_annotations = {k: collapse_subtokens(v, flattened_token_mapping[k[1]]) for k, v in
                            doc_soft_annotations.items()}
    doc_hard_annotations = {k: collapse_subtokens(v, flattened_token_mapping[k[1]]) for k, v in
                            doc_hard_annotations.items()}
    pred_rationales = []
    for ((ann_id, docid), hard_ann), inst in zip(doc_hard_annotations.items(), input_data):
        start_token = 1
        while start_token < len(hard_ann) - 1:
            start_token += 1
            if hard_ann[start_token] == 1:
                end_token = start_token
                while end_token < len(hard_ann):
                    end_token += 1
                    if end_token == len(hard_ann) or hard_ann[end_token] == 0:
                        break
                pred_rationales.append(Rationale(ann_id, docid,
                                                 start_token,
                                                 end_token))
    ...
    results['hard_rationale_scores(rational_prf)'] = score_hard_rationale_predictions(true_rationales, pred_rationales)
    results['hard_rationale_partial_match_scores(iou_scores)'] = partial_match_score(true_rationales, pred_rationales,
                                                                                     [0.5])

    token_level_truth = list(chain.from_iterable(rat.to_token_level() for rat in true_rationales))
    token_level_pred = list(chain.from_iterable(inst.to_token_level() for inst in pred_rationales))
    token_level_prf = score_hard_rationale_predictions(token_level_truth, token_level_pred)
    results['token_prf'] = token_level_prf

    # soft rationale scoring
    instance_format = []
    for (ann_id, docid), soft_ann in doc_soft_annotations.items():
        instance_format.append({
            'annotation_id': ann_id,
            'rationales': [{
                'docid': docid,
                'soft_rationale_predictions': soft_ann,
            }],
        })
    flattened_documents = {k: list(chain.from_iterable(v)) for k, v in documents.items()}
    ...
    token_scoring_format = PositionScoredDocument.from_results(instance_format, truth, flattened_documents,
                                                               use_tokens=True)
    results['soft_token_scores'] = score_soft_tokens(token_scoring_format)
    sentence_scoring_format = PositionScoredDocument.from_results(instance_format, truth, flattened_documents,
                                                                  use_tokens=True)
    results['soft_sentence_scores'] = score_soft_tokens(sentence_scoring_format)
    return results
...
def decode_evidence_tokens_and_classify(evidence_token_identifier: nn.Module,
    ...
    def decode_batch(data: List[Tuple[SentenceEvidence, SentenceEvidence]], name: str, score: bool = False,
        ...
        with torch.no_grad():
            ...
            _, soft_identification_preds, hard_identification_preds, id_preds_truth = make_token_preds_epoch(
