...
class Evidence:
    ...
    """
    text: Union[str, Tuple[int], Tuple[str]] #text can be either strings or Tuple of Integers or Tuple of Strings
    docid: str
    start_token: int = -1
    end_token: int = -1
...
#-------Defining a class for Annotation-------#
@dataclass(eq=True, frozen=True)
class Annotation:
    """
    Args:
    ...
    #----this function takes set and frozenset iterable objets and first group them into list then tuple----#
    def all_evidences(self) -> Tuple[Evidence]:
        return tuple(list(chain.from_iterable(self.evidences)))



#---Converting ANNOTATIONS to JSON file and save the JSON in output_file---#
def annotations_to_jsonl(annotations, output_file):
    with open(output_file, 'w') as of:
        for ann in sorted(annotations, key=lambda x: x.annotation_id):
            ...
            as_str = json.dumps(as_json, sort_keys=True) #Converting a dictionary to Json file
            of.write(as_str)
            of.write('\n')
...
#----RECURSION PROCESS IS GOING TO BE HAPPENED----#
def _annotation_to_dict(dc):
    # convenience method
    if is_dataclass(dc):  #if the Object is data_class then it return a dictionary
        d = asdict(dc)
        ret = dict()
        for k, v in d.items():
...
#---CONVERTING THE JSON FILE TO LIST OF DICTIONARY---#
def load_jsonl(fp: str) -> List[dict]:
    ret = []
    with open(fp, 'r') as inf:
        for line in inf:
            content = json.loads(line) #convert JSON TO DICTIONARY
            ret.append(content)
    return ret

#---CONVERTING DICT TO THE JSON FILE---#
...
#---CONVERTING JSON FILE to ANNOTATION DATACLASS FORMAT---#
def annotations_from_jsonl(fp: str) -> List[Annotation]:
    ...
    with open(fp, 'r') as inf:
        for line in inf:
            ...
            ev_groups = []
            #TUPLE COMPREHENSION IS DONE THEN CONVERTS IT INTO TUPLE and store it into ev_groups[] list
...
#--- CHECKING THE DOC IDS ARE PRESENT IN THE ANNOTATIONS ARE NOT ---#
...
def load_datasets(data_dir: str) -> Tuple[List[Annotation], List[Annotation], List[Annotation]]:
    ...
    #---train_data, val_data and test_data all are LIST of Annotations---#
...
def load_documents(data_dir: str, docids: Set[str] = None) -> Dict[str, List[List[str]]]:
    ...
    for d in docids:
        with open(os.path.join(docs_dir, d), 'r') as inf:
            lines = [l.strip() for l in inf.readlines()]  #strip() function removes white space before and after a string
            lines = list(filter(lambda x: bool(len(x)), lines)) #filter function is used to remove all empty strings from lines
            #Taking each line of sentences tokenizing them
